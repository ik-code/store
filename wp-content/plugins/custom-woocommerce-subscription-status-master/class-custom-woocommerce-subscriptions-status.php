...
class Custom_Woocommerce_Subscription_Status
{
    ...
    /**
     * Registered new status by adding `Canceled by user` to $registered_statuses array.
     *
     * @access public
     *
    ...
    {
        $registered_statuses['wc-canceled-by-user'] = _nx_noop('Canceled by user <span class="count">(%s)</span>', 'Canceled by user <span class="count">(%s)</span>', 'post status label including post count', 'custom-wcs-status-texts');
        return $registered_statuses;
    }

    /**
     * Add new status `Canceled by user` to $subscription_statuses array.
     *
     * @access public
     *
    ...
    {
        $subscription_statuses['wc-canceled-by-user'] = _x('Canceled by user', 'Subscription status', 'custom-wcs-status-texts');
    ...
    {
        if ($new_status == 'canceled-by-user') {
     ...
     * This code will also activate `reactivate` link in the list page for the subscription with `Canceled by user` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_reactivation') && $subscription->has_status(array('on-hold', 'canceled-by-user'))) {
            $can_be_updated = true;
     ...
     * This function replaces the default code with the new one.
     * This code will also activate `suspend` link in the list page for the subscription with `Canceled by user` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_suspension') && $subscription->has_status(array('active', 'pending', 'canceled-by-user'))) {
            $can_be_updated = true;
        } else {
            $can_be_updated = false;
        }
    ...
    {
        $bulk_actions['canceled-by-user'] = _x('Mark Canceled by user', 'an action on a subscription', 'custom-wcs-status-texts');
        return $bulk_actions;
    }
